...
function aicaAppendConditionQR(replyText) {
  ...
  // Patch 7: 質問の文脈を厳格に判定。QR選択肢ずれを防ぐ
  ...
  // 強み（Patch 7: 具体例を提示）
  if (/(自信.*?スキル|得意|強み).*?(教えて|伺|お聞き|どんな|何)/.test(t) && !/弱み|分野|施設/.test(t)) {
    return { items: [
      qr("アセスメント", "アセスメント"),
      qr("急変対応", "急変対応"),
  ...
  // 苦手・避けたい
  if (/(苦手|避け).*?(教えて|伺|お聞き|どんな|何|具体的)/.test(t)) {
    return { items: [
      qr("夜勤自体", "夜勤自体が苦手"),
      qr("急変対応", "急変対応が苦手"),
  ...
  // 希望施設タイプ
  if (/希望.*?(施設|病棟|職場|病院)|どのよう.*?職場|どんな.*?施設/.test(t) && !/通勤|給与|働き方|分野/.test(t)) {
    return { items: [
      qr("急性期病院", "急性期病院"),
      qr("回復期病院", "回復期病院"),
      qr("療養病院", "療養病院"),
  ...
  // 働き方
  if (/働き方|勤務形態|常勤.*?非常勤|フルタイム|日勤.*?夜勤.*?選/.test(t) && !/通勤|給与|施設|分野/.test(t)) {
    return { items: [
      qr("日勤のみ", "日勤のみ"),
      qr("二交代夜勤あり", "二交代夜勤あり"),
      qr("三交代夜勤あり", "三交代夜勤あり"),
  ...
  // 夜勤回数
  if (/夜勤.*?(回|月).*?(?:何|どの|教えて|伺)|月何回|何回まで/.test(t)) {
    return { items: [
      qr("月3回まで", "月3回まで"),
      qr("月4〜5回", "月4〜5回"),
      qr("月6〜8回", "月6〜8回"),
  ...
  // 希望エリア（Patch 7: 郵便番号入力誘導）
  if (/(希望.*?エリア|勤務地|どこ.*?(働|勤務)).*?(教えて|伺|お聞き)/.test(t) && !/施設|通勤|働き方/.test(t)) {
    return { items: [
  ...
  // 通勤手段（Patch 7: 必ず「方法」「手段」が含まれる時のみ）
  if (/通勤.*?(方法|手段)|電車.*?(?:や|か).*?車|車.*?(?:や|か).*?電車|どのよう.*?通勤/.test(t) && !/施設|エリア/.test(t)) {
    return { items: [
      qr("電車", "電車通勤"),
      qr("車", "車通勤"),
      qr("自転車", "自転車通勤"),
  ...
  // 給与
  if (/(給与|年収|月給|手取り|希望.*?収入).*?(教えて|伺|お聞き|どの|希望|教えて|具体的)/.test(t) && !/通勤|施設|働き方/.test(t)) {
    return { items: [
      qr("現状維持", "現状維持で良い"),

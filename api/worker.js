...
もしよろしければ、転職についてのお話を続けることもできます。今のお気持ち次第で選んでください。`;

// Patch 11: 電話予約 Flex Carousel（平日9-16時 × 次の10営業日）
function aicaBuildBookingFlex() {
  ...
  while (slots.length < 10) {
    const dow = cursor.getUTCDay(); // 0=Sun, 6=Sat (UTC base date)
    if (dow !== 0 && dow !== 6) {
      const iso = cursor.toISOString().slice(0, 10);
      const m = cursor.getUTCMonth() + 1;
      const d = cursor.getUTCDate();
      const dowLabel = ["日","月","火","水","木","金","土"][dow];
      slots.push({ iso, label: `${m}/${d}（${dowLabel}）` });
    }
    cursor.setUTCDate(cursor.getUTCDate() + 1);
  }
  ...
  const bubbles = slots.map(slot => ({
    ...
    header: {
      ...
      contents: [
        { type: "text", text: slot.label, weight: "bold", size: "lg", color: "#FFFFFF", align: "center" },
      ],
    },
    body: {
      type: "box",
      ...
      contents: times.map(t => ({
        ...
        action: {
          type: "postback",
          label: t,
          data: `book=${slot.iso}T${t}`,
          displayText: `${slot.label} ${t} で予約`,
        },
      })),
    },
  }));
...
// Patch 7: 緊急ワード後の選択肢（質問継続 or 今は休む）
function aicaBuildEmergencyQuickReply() {
  const qr = (label, data) => ({ type: "action", action: { type: "postback", label, data, displayText: label } });
  return {
    items: [
      qr("話を続ける", "fallback=continue_aica"),
      qr("今は休む", "fallback=pause_aica"),
    ],
  };
}
...
・常に丁寧で温かいトーン。ただし**過剰共感は禁止**
・「聞いてくれている」と感じてもらえる応答を心がける
・**毎ターン、軽く受け止めの一文を入れる**（重すぎる共感は不快感を生む）
  ◎ 良い受け止め: 「夜勤の負担、お話くださってありがとうございます」「なるほど、そういう状況なのですね」
  ◎ 良いリフレクション: ユーザー「夜勤が人より多くて」→ AI「夜勤の偏りがあるんですね」
  ✕ 過剰NG: 「本当に大変だったことと思います」「想像するだけで胸が痛みます」（やってないのに分かる風は嫌われる）
  ✕ 過剰NG: 「お辛いですよね」を毎ターン繰り返す（最初の1回まで）
...
⚠️ **is_closing 判定ルール**:
- Turn 4 で「質問」だけを送る場合 is_closing = false（ユーザーの返答待ち）
- ユーザーの Turn 4 への返答を受け取った後で is_closing = true（要約とクロージング）
- 質問とクロージングを同じターンで返さない（連続投稿でユーザーが返答する間がなくなるため）

---

【深掘り質問のフレームワーク】（受け止めの後に1問だけ）

⚠️ 重要: ユーザーが Turn 1 ですでに具体的な場面・原因を詳しく話している場合、
Turn 2 で「具体的にどんな場面で？」と再度聞くのは禁止（既出のため）。
その場合は Turn 2 を「環境・構造の特定」にスキップしてOK。

⚠️ Turn 2 と Turn 3 で似た「もう限界の場面」を繰り返さない。
Turn 2 で構造を聞いたなら、Turn 3 は「理想・条件」へ前倒し可能。

Turn 1（きっかけの言語化）:
  軽く受け止め、その後:
  「もう少し具体的にお聞かせください。それは特定の方との関係ですか、
   それとも職場全体の空気でしょうか？」（=Turn 2の質問を前倒しでOK）

Turn 2（環境・構造の特定、軸別。Turn 1の答えで既出なら次へ）:
  受け止めの後:
...
Turn 3（理想像・譲れない条件）:
  ...
  「もし環境が整っていれば、○○さんが『続けたい』と感じるのは、
   どんな条件が揃っている時でしょうか？」
  （※「もう限界の場面」は Turn 1〜2 で既出の可能性が高いため、原則聞かない）

Turn 4（クロージング判定）:
  3ターンで根本原因が把握できていれば Turn 4 はクロージング（is_closing=true）。
  まだ不明な点があれば1問だけ追加質問。
...
【統計引用ルール（厳守）】
- 出典が確認できない統計データは絶対に引用しない。
- 「厚労省○○調査」「○○年データ」等の権威付けを伴う数値は、ハルシネーションが起きやすいため使用禁止。
- 不確かな場合は「公的調査では○○程度のレンジが報告されています」と幅で示す（出典の特定なし）。
- 個別交渉での具体数値（賞与○ヶ月、月給○万円等）は施設DB（D1.facilities / D1.jobs）の実データのみ使用。
- 想像で数字を作ることは絶対禁止。
...
// IL→AICA 共感ブリッジ用: 感情的な venting テキストかを判定。
// 短文 (postback ラベル相当) や住所/施設名のような事実テキストは除外。
function isEmotionalVentingText(text) {
  if (!text || typeof text !== "string") return false;
  const t = text.trim();
  if (t.length < 10) return false; // postback label相当の短文は除外
  // 緊急キーワード (即 emergency 扱いだが、ここでは検出のみ)
  if (/死にたい|消えたい|自殺|パワハラ|セクハラ|モラハラ|いじめ|DV|虐待/.test(t)) return true;
  // 感情キーワード辞書 (現場語彙含む)
  if (/辛い|つらい|しんどい|きつい|厳しい|大変|限界|無理|もう|泣/.test(t)) return true;
  if (/疲れ|疲労|疲弊|燃え尽き|バーンアウト/.test(t)) return true;
  if (/辞めたい|やめたい|退職したい/.test(t)) return true;
  if (/不安|心配|怖い|恐怖/.test(t)) return true;
  if (/ストレス|嫌がらせ|いやがらせ/.test(t)) return true;
  if (/病んで|鬱|うつ気味|病む/.test(t)) return true;
  // 看護師の現場語彙が含まれている長め文
  if (t.length >= 15 && /夜勤|プリセプター|師長|主任|申し送り|ナースコール|二交代|三交代|日勤/.test(t)) return true;
  // 評価/成長/家庭などのテーマ + 文末「ない/ます/です」 → 自由文 venting と推定
  if (t.length >= 18 && /成長|評価|頑張|認定|キャリア|家族|子供|旦那|夫|妻|親|介護/.test(t)
      && /(ない|ません|です|ます|たい|だ|た|よ)\s*[。.…!！？?]?\s*$/.test(t)) return true;
  return false;
}
...
function aicaBuildWelcomeMessage(displayName) {
  ...
  // Patch 7: 社長指示で文言・改行を全面リライト
  return `${displayName ? name + "、こ" : "こ"}んにちは🌸 ナースロビーAIキャリアアドバイザーです。

私は転職に特化したAIです。24時間いつでも、誰にも知られずにお仕事のお話を伺えます。

最大4つの質問で、あなたの「本当に必要な条件」を整理した後、具体的な求人をご提案します✨

🎙️ 入力が大変な時は、LINEのマイクボタンをタップして送信するだけで音声を読み込んでお答えします。
...
function aicaAppendConditionQR(replyText) {
  ...
  // Patch 10 (2026-04-30): 全パターン排他条件を厳格化
  // 問題: AIは前ターンの内容を recap（「管理職経験あるんですね、では次に苦手なことを」）
  //       するため、複数トピックの単語が混ざる → 役割QRが苦手質問で誤発火
  // 解決: 「最後の質問文（末尾300文字 + 末尾?周辺）」に絞ってマッチング判定
  //       各パターンに「他トピックが末尾に含まれない」否定条件を追加
  const tail = fullText.slice(-400);  // 末尾400文字に絞って現在の質問を識別

  // type=message のQR項目（タップで送信されるテキスト）
  const qr = (label, text) => ({ type: "action", action: { type: "message", label, text } });
  // 自由入力ボタン（Patch 9 と同じく postback + キーボード起動。AI誤発火防止）
  const qrInput = { type: "action", action: { type: "postback", label: "自分で入力", data: "aica_open_keyboard", displayText: " ", inputOption: "openKeyboard", fillInText: "" } };

  // 順序: 末尾質問の主トピック判定（最も末尾近くにある質問語を優先）
  // 経験年数 → 役割 → 分野 → 強み → 弱み → 施設タイプ → 働き方 → 夜勤回数 → エリア → 通勤 → 給与 → 時期

  // 経験年数（Patch 8: QR廃止・数字フリー入力）
  if (/何年目|看護師.*?何年|経験.*?年数|経験年数|看護師としては.*?年/.test(tail) &&
      !/役割|施設|希望|働き方|通勤|給与|分野|強み|得意|苦手|避け|弱み|時期|エリア|夜勤回数/.test(tail)) {
    return null;
  }

  // 苦手・避けたい（Patch 10: 役割より前に判定。「苦手」キーワードを優先）
  if (/(苦手|避けたい|避ける|嫌|やりたくない).*?(教えて|伺|お聞き|どんな|何|具体的|ですか)/.test(tail) &&
      !/強み|得意|施設|希望|分野|働き方|エリア|給与|通勤|時期|夜勤回数|現在の役割/.test(tail)) {
    return { items: [
      qr("夜勤自体", "夜勤自体が苦手"),
      qr("急変対応", "急変対応が苦手"),
      qr("特定診療科", "特定の診療科を避けたい"),
      qr("オンコール", "オンコールを避けたい"),
      qr("管理業務", "管理業務を避けたい"),
      qr("こだわらない", "特に苦手はない"),
      qrInput,
  ...
  // 強み（Patch 10: 苦手の後に判定。「強み」「得意」キーワードを「苦手」と区別）
  if (/(自信.*?スキル|得意|強み).*?(教えて|伺|お聞き|どんな|何|ですか)/.test(tail) &&
      !/苦手|避け|弱み|施設|希望|分野|働き方|エリア|給与|通勤|時期|夜勤回数|役割/.test(tail)) {
    return { items: [
      qr("アセスメント", "アセスメント"),
      qr("急変対応", "急変対応"),
      qr("採血・点滴", "採血・点滴・ルート確保"),
      qr("人工呼吸器", "人工呼吸器管理"),
      qr("化学療法", "化学療法"),
      qr("ターミナル", "ターミナルケア"),
      qr("家族対応", "家族対応・コミュニケーション"),
      qr("教育・指導", "新人教育・指導"),
      qrInput,
    ]};
  }

  // 役割（Patch 10: 「苦手」「強み」が末尾になければ役割と判定）
  // 「役割について教えてください」「現在の役割は」「リーダー経験はありますか」等が末尾にある時のみ
  if (/(現在の役割|役割.*?(教えて|伺|お聞き|ですか|何))|(?:リーダー|プリセプター|主任|師長|管理職).*?(?:経験|でしたか|ですか)|ポジション.*?(?:教えて|伺)/.test(tail) &&
      !/苦手|避け|強み|得意|施設|希望|分野|働き方|エリア|給与|通勤|時期|夜勤回数|何年目/.test(tail)) {
    return { items: [
      qr("一般スタッフ", "一般スタッフ"),
  ...
  // 経験した科・分野（Patch 10: 「苦手」「強み」「役割」が末尾になければ）
  if (/(科|分野|診療科|専門領域).*?(経験|教えて|伺|どんな|ですか)|どんな.*?(科|分野)|これまで.*?(科|分野)/.test(tail) &&
      !/苦手|避け|強み|得意|役割|施設|希望|働き方|エリア|給与|通勤|時期|夜勤回数/.test(tail)) {
    return { items: [
      { type: "action", action: { type: "postback", label: "自分で書く（推奨）", data: "aica_open_keyboard", displayText: " ", inputOption: "openKeyboard", fillInText: "" } },
  ...
  // 希望施設タイプ
  if (/希望.*?(施設|病棟|職場|病院)|どのよう.*?(職場|施設)|どんな.*?(施設|職場)/.test(tail) &&
      !/苦手|避け|強み|得意|役割|分野|働き方|エリア|給与|通勤|時期|夜勤回数/.test(tail)) {
    return { items: [
      qr("急性期病院", "急性期病院"),
      qr("回復期病院", "回復期病院"),
      qr("療養病院", "療養病院"),
      ...
      qr("訪問看護", "訪問看護"),
      qr("介護施設", "介護施設"),
      qr("両方/迷っている", "急性期と訪問看護など複数気になっている"),
      qrInput,
    ]};
  }

  // 働き方
  if (/働き方|勤務形態|常勤.*?非常勤|フルタイム|日勤.*?夜勤.*?選/.test(tail) &&
      !/苦手|避け|強み|得意|役割|施設|希望.*?施設|分野|エリア|給与|通勤|時期|夜勤回数/.test(tail)) {
    return { items: [
      qr("日勤のみ", "日勤のみ"),
      qr("二交代夜勤あり", "二交代夜勤あり"),
      qr("三交代夜勤あり", "三交代夜勤あり"),
  ...
  // 夜勤回数
  if (/夜勤.*?(回|月).*?(?:何|どの|教えて|伺|ですか)|月何回|何回まで/.test(tail) &&
      !/苦手|避け|強み|得意|役割|施設|希望|分野|エリア|給与|通勤|時期/.test(tail)) {
    return { items: [
      qr("月3回まで", "月3回まで"),
      qr("月4〜5回", "月4〜5回"),
      qr("月6〜8回", "月6〜8回"),
  ...
  // 希望エリア
  if (/(希望.*?エリア|勤務地|どこ.*?(働|勤務)).*?(教えて|伺|お聞き|ですか)/.test(tail) &&
      !/苦手|避け|強み|得意|役割|施設|分野|働き方|給与|通勤|時期|夜勤回数/.test(tail)) {
    return { items: [
      qr("郵便番号で送る", "郵便番号を入力します"),
      qr("駅名で送る", "駅名を入力します"),
      qr("市区町村", "市区町村を入力します"),
      qr("こだわらない", "エリアこだわらない"),
      qrInput,
  ...
  // 通勤手段
  if (/通勤.*?(方法|手段)|電車.*?(?:や|か).*?車|車.*?(?:や|か).*?電車|どのよう.*?通勤/.test(tail) &&
      !/苦手|避け|強み|得意|役割|施設|希望.*?施設|分野|働き方|エリア|給与|時期|夜勤回数/.test(tail)) {
    return { items: [
      qr("電車", "電車通勤"),
      qr("車", "車通勤"),
      qr("自転車", "自転車通勤"),
  ...
  // 給与
  if (/(給与|年収|月給|手取り|希望.*?収入).*?(教えて|伺|お聞き|どの|希望|具体的|ですか)/.test(tail) &&
      !/苦手|避け|強み|得意|役割|施設|分野|働き方|エリア|通勤|時期|夜勤回数/.test(tail)) {
    return { items: [
      qr("現状維持", "現状維持で良い"),
  ...
  // 時期
  if (/転職.*?時期|転職.*?いつ|いつ頃|いつまで|いつから/.test(tail) &&
      !/苦手|避け|強み|得意|役割|施設|分野|働き方|エリア|通勤|給与|夜勤回数/.test(tail)) {
    return { items: [
      qr("すぐ", "すぐ転職したい"),
      qr("3ヶ月以内", "3ヶ月以内"),
      qr("半年以内", "半年以内"),
...
async function aicaProcessTextBackground({ userId, userText, channelAccessToken, env }) {
  ...
  if (["aica_turn1", "aica_turn2", "aica_turn3", "aica_turn4"].includes(entry.phase)) {
    ...
    if (result.isEmergency) {
      ...
      // ユーザーに「話を続ける/今は休む」のQuickReplyで判断させる
      entry.phase = "emergency_paused";
      await saveLineEntry(userId, entry, env);
      await pushTo([{ type: "text", text: result.reply, quickReply: aicaBuildEmergencyQuickReply() }]);
      if (env.SLACK_BOT_TOKEN) {
        const nowJST = new Date().toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" });
        await fetch("https://slack.com/api/chat.postMessage", {
          method: "POST",
          headers: { "Authorization": `Bearer ${env.SLACK_BOT_TOKEN}`, "Content-Type": "application/json; charset=utf-8" },
          body: JSON.stringify({ channel: env.SLACK_CHANNEL_ID || "C0AEG626EUW", text: `🚨 *AICA緊急キーワード検出*\nユーザー: \`${userId}\`\nメッセージ: ${userText.slice(0, 200)}\n時刻: ${nowJST}\n※ ユーザーに「話を続ける/今は休む」の選択肢提示中` }),
        }).catch(() => {});
      }
      return;
    }
...
// ---------- 外部公開求人データ（ハローワークAPI 2026-04-30更新） ----------
...
const EXTERNAL_JOBS = {
  nurse: {
    ...
    "藤沢": [
      ...
      {n:"社会福祉法人　光友会", t:"看護師／正社員（湘南希望の郷）", r:"A", s:74, d:{sal:25,hol:10,bon:15,emp:15,wel:1,loc:8}, sal:"月給27.5〜30.5万円", sta:"湘南台駅（西口）からバス「光友会入口」下車　...", hol:"114日", bon:"4.0ヶ月", emp:"正社員", wel:"車通勤可", desc:"入所している障害者への看護業務。主な業務内容は、　　＊日々の投薬　　　　　　　＊バイタルチェック　　＊医療的処置　　　　　　　＊嘱託医の診察補助　　＊医療機関との連絡調整　　※応募前の施設見学歓迎。事前にご連絡下さい。　　　　　　【変更の範囲】変更なし", loc:"神奈川県藤沢市", shift:"(1)8時30分～17時30分", ctr:"雇用期間の定めなし", ins:"雇用　労災　健康　厚生"},
    ...
    "さいたま": [
      ...
      {n:"株式会社　大双", t:"看護師（緑区）", r:"A", s:71, d:{sal:30,hol:17,bon:4,emp:15,wel:3,loc:2}, sal:"月給25.0〜45.0万円", sta:"ＪＲ東浦和", hol:"123日", bon:"あり", emp:"正社員", wel:"車通勤可、住宅手当/寮", desc:"訪問看護師として、在宅で生活されている利用者様への医療的ケア、生活療養上の助言やサポート、日常生活を安心・安全に過ごして頂くための住宅、環境整備。その他職種のサービスを含め円滑に利用者様が生活できるように多職種との連携をしながら訪問看護サービスを提供します。　　【変更範囲：変更なし】", loc:"埼玉県さいたま市緑区", shift:"(1)9時00分～18時00分", ctr:"雇用期間の定めなし", ins:"雇用　労災　健康　厚生"},
      ...
      {n:"株式会社　明昭", t:"看護スタッフ（大宮明生苑）正看護師／さいたま市北区", r:"A", s:65, d:{sal:15,hol:17,bon:15,emp:15,wel:1,loc:2}, sal:"月給23.0万円", sta:"ニューシャトル　「鉄道博物館」", hol:"120日", bon:"4.0ヶ月", emp:"正社員", wel:"車通勤可", desc:"有料老人ホーム「大宮明生苑」に御入居されている利用者様の　健康管理や医療処置等の看護業務。　　※経験者歓迎　　ブランクがあっても応募可能です。　　「変更範囲：変更なし」", loc:"埼玉県さいたま市北区", shift:"(1)7時20分～15時55分 / (2)8時45分～17時20分", ctr:"雇用期間の定めなし", ins:"雇用　労災　健康　厚生"},
    ...
    "川口・戸田": [
      ...
      {n:"医療法人社団　城東桐和会", t:"看護師／中島病院／戸田公園駅から徒歩１２分", r:"A", s:65, d:{sal:30,hol:17,bon:0,emp:15,wel:1,loc:2}, sal:"月給28.8〜37.0万円", sta:"埼京線　戸田公園", hol:"121日", bon:"なし", emp:"正社員", wel:"車通勤可", desc:"戸田市の病院での看護師業務全般をお任せします　・バイタルチェック、ラウンド　・注射、点滴、採血、経管栄養、ＣＶポート、ＩＶＨなどの処置　・患者様の身体管理、服薬管理、整容、介助　・カルテ記録（電子カルテ）　・看護計画の立案、実施　・看護職間のミーティング、他職種とのカンファレンス　・ナースコール対応", loc:"埼玉県戸田市", shift:"(1)8時45分～17時45分 / (2)16時15分～9時15分", ctr:"雇用期間の定めなし", ins:"雇用　労災　健康　厚生"},
      ...
      {n:"株式会社　明昭", t:"看護スタッフ（グランヴィ川口）／正看護師／川口市", r:"B", s:63, d:{sal:15,hol:17,bon:15,emp:15,wel:1,loc:0}, sal:"月給23.0万円", sta:"", hol:"120日", bon:"4.0ヶ月", emp:"正社員", wel:"車通勤可", desc:"有料老人ホーム「グランヴィ川口」に御入居されている利用者様の　健康管理や医療処置等の看護業務。　　※経験者歓迎　　ブランクがあっても応募可能です。　　　「変更範囲：変更なし」", loc:"埼玉県川口市", shift:"(1)7時45分～16時20分 / (2)8時45分～17時20分", ctr:"雇用期間の定めなし", ins:"雇用　労災　健康　厚生"},
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // ============ Patch 11: キャリアシートフォーム送信受付 ============
    if (url.pathname === "/api/career-sheet" && request.method === "POST") {
      try {
        const body = await request.json();
        const { userId, jobId, jobName } = body;
        ...
        if (env?.LINE_SESSIONS) {
          await env.LINE_SESSIONS.put(`career_sheet:${userId}`, JSON.stringify(sheetData));
        }
        // entry更新（phase = inquire_booking_pending）
        const entry = await getLineEntryAsync(userId, env);
        if (entry) {
          entry.careerSheetCompleted = true;
          entry.careerSheetAt = Date.now();
          entry.phase = "inquire_booking_pending";
          await saveLineEntry(userId, entry, env);
        }
        // LINE Push: 完了通知 + 電話予約Flex
        const pushToken = env.LINE_CHANNEL_ACCESS_TOKEN;
        if (pushToken && entry) {
          const bookingFlex = aicaBuildBookingFlex();
          ctx.waitUntil(fetch("https://api.line.me/v2/bot/message/push", {
            method: "POST",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${pushToken}` },
            body: JSON.stringify({
              to: userId,
              messages: [
                { type: "text", text: `キャリアシート、ありがとうございます🎉\n\nマイページに保存しました📝\n\n最後に、担当者と電話で5分お話しさせてください。\n平日9:00〜16:00で、ご都合の良い日時を選んでください📞` },
                bookingFlex,
              ],
            }),
          }).catch(e => console.error(`[CareerSheet] LINE push error: ${e.message}`)));
        }
        // Slack通知
        if (env.SLACK_BOT_TOKEN) {
          const nowJST = new Date().toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" });
        ...
        return jsonResponse({ ok: true, savedAt: sheetData.submittedAt });
      } catch (e) {
        console.error(`[CareerSheet] error: ${e.message}`);
        return jsonResponse({ error: e.message }, 500);
      }
    }
...
function handleLinePostback(dataStr, entry) {
  ...
  // 想定外テキスト用フォールバック選択肢
    ...
    if (val === "restart") {
      ...
      delete entry.aicaProfile;
      nextPhase = "aica_turn1";
    } else if (val === "pause_aica") {
      // Patch 7: 緊急ワード後「今は休む」→ handoff（沈黙）
      nextPhase = "handoff";
    }
  }
  // 応募同意
  ...
  try {
    for (const event of events) {
      ...
      if (event.type === "follow") {
        ...
        {
          // Patch 6 (D): source計測タグ強化
          // 旧: 不明時 'none' → 全件none扱いで流入経路が見えなかった
          // 新: preloadedCtx.source（utm_source由来）→ 'shindan'（LP診断）→ 'direct'（直接follow）
          if (!entry.welcomeSource) {
            if (preloadedCtx) {
              entry.welcomeSource = preloadedCtx.source || (liffSessionCtx ? 'liff' : 'shindan');
            } else {
              entry.welcomeSource = 'direct';
            }
          }
          entry.phase = "aica_turn1";
          entry.aicaTurnCount = 0;
          ...
          await saveLineEntry(userId, entry, env);
          // Patch 6 (A): Welcome に AICA軸誘導の QuickReply を付与
          await lineReply(event.replyToken, [{
            type: "text",
            text: aicaBuildWelcomeMessage(entry.aicaDisplayName),
            quickReply: aicaBuildWelcomeQuickReply(),
          }], channelAccessToken);
      ...
      if (event.type === "postback") {
        ...
        // ============ Patch 11: 📨 問い合わせる postback handler ============
        // 求人カード「📨 問い合わせる」→ キャリアシートフォーム誘導 → 電話予約フロー
        if (dataStr.startsWith("inquire_job=")) {
          const params = new URLSearchParams(dataStr);
          const jobId = (params.get("inquire_job") || "unknown").slice(0, 100);
          const decodedJobId = decodeURIComponent(jobId);

          // 求人情報の解決（matchingResults / lastShownJobs / D1）
          let jobName = "ご希望の求人";
          try {
            const candidates = [...(entry.matchingResults || []), ...(entry.lastShownJobs || [])];
            const match = candidates.find(r => {
              const ids = [r.jobId, r.id, r.n, r.employer];
              return ids.some(c => c != null && String(c) === decodedJobId);
            });
            if (match) {
              jobName = (match.employer || match.n || match.title || match.t || "ご希望の求人").slice(0, 100);
            } else if (/^\d+$/.test(decodedJobId) && env.DB) {
              const row = await env.DB.prepare(
                'SELECT employer, title FROM jobs WHERE id = ? LIMIT 1'
              ).bind(parseInt(decodedJobId, 10)).first();
              if (row) jobName = (row.employer || row.title || jobName).slice(0, 100);
            }
          } catch (e) { /* ignore */ }

          entry.inquireJobId = decodedJobId;
          entry.inquireJobName = jobName;
          entry.inquireStartedAt = Date.now();
            ...
            },
            {
              ...
              contents: {
                ...
                body: {
                  ...
                  paddingAll: "16px",
                  contents: [
                    { type: "text", text: "📝 キャリアシート作成", weight: "bold", size: "lg", color: "#E8756D" },
                    { type: "text", text: "所要時間: 約3分", size: "sm", color: "#888888", margin: "sm" },
                  ],
                },
                footer: {
                  type: "box",
          ...
          if (env.SLACK_BOT_TOKEN) {
            ...
            ctx.waitUntil(fetch("https://slack.com/api/chat.postMessage", {
              ...
              body: JSON.stringify({
                channel: env.SLACK_CHANNEL_ID || "C0AEG626EUW",
                text: `📨 *求人問い合わせ開始*\nユーザー: \`${userId}\`\n求人: ${jobName}\n時刻: ${nowJST}\nキャリアシートフォーム送付済。完了したら電話予約UI Push予定。`,
              }),
            }).catch(() => {}));
          }
          continue;
        ...
        // ============ Patch 11: 📞 電話予約完了 postback handler ============
        // book=YYYY-MM-DDTHH:00 形式のpostback
        if (dataStr.startsWith("book=")) {
          const datetimeStr = dataStr.slice("book=".length);
          // バリデーション: YYYY-MM-DDTHH:MM
          if (!/^\d{4}-\d{2}-\d{2}T\d{1,2}:\d{2}$/.test(datetimeStr)) {
            await lineReply(event.replyToken, [{ type: "text", text: "予約日時の形式が正しくありません。もう一度日時をお選びください🌸" }], channelAccessToken);
            continue;
          }
          entry.bookingDatetime = datetimeStr;
          entry.bookingAt = Date.now();
          entry.phase = "booked";
          await saveLineEntry(userId, entry, env);
          ...
          const dowLabel = ["日","月","火","水","木","金","土"][dateObj.getUTCDay()];
          const datetimeJa = `${m}月${d}日（${dowLabel}）${timePart}`;

          await lineReply(event.replyToken, [{
            type: "text",
            text: `ありがとうございます！\n\n${datetimeJa} に担当者からお電話いたします📞\n\n事前にキャリアシートを拝見し、ご希望に沿ったお話ができるよう準備してお待ちしております🌸\n\n変更や追加のご質問があれば、いつでもメッセージください。`,
          }], channelAccessToken);
          ...
          if (env.SLACK_BOT_TOKEN) {
            ...
            ctx.waitUntil(fetch("https://slack.com/api/chat.postMessage", {
              ...
              body: JSON.stringify({
                channel: env.SLACK_CHANNEL_ID || "C0AEG626EUW",
                text: `📞 *新規電話予約*\n${nameLine}userId: \`${userId}\`\n📅 *${datetimeJa}*\n求人: ${entry.inquireJobName || entry.inquireJobId || "—"}\nキャリアシート: ${entry.careerSheetCompleted ? '✅完成' : '❌未完成'}\n時刻: ${nowJST}\n\n💬 \`!reply ${userId} メッセージ\``,
              }),
            }).catch(() => {}));
          }
          continue;
...
- 700-1000文字で作成`;
      ...
      if (event.type === "message" && event.message.type === "text") {
        ...
        // ===== IL→AICA 共感ブリッジ =====
        // IL flow phase で自由テキストが感情的 venting だった場合、
        // 冷たい「もう一度お選びください」の代わりに AICA 心理ヒアリング (aica_turn1) に転回。
        // 社長指摘『寄り添ってくれている気がしない』への対応。
        const _IL_BRIDGE_PHASES = new Set([
          "il_area", "il_subarea", "il_facility_type",
          "il_department", "il_workstyle", "il_urgency",
        ]);
        if (entry.phase && _IL_BRIDGE_PHASES.has(entry.phase) && isEmotionalVentingText(userText)) {
          ...
          // フォールスルー → 直下の AICA フロー処理ブロックで通常通り処理される
        }
          ...
          if (["aica_turn1", "aica_turn2", "aica_turn3", "aica_turn4"].includes(entry.phase)) {
            ...
            ctx.waitUntil((async () => {
              try {
                ...
                if (result.isEmergency) {
                  // Patch 7: BG処理側でも emergency_paused へ。QuickReply付きで送信
                  _entry.phase = "emergency_paused";
                  updateLastAuditTrail(_entry, { phaseAfter: "emergency_paused", appendReplyTexts: [result.reply] });
                  await saveLineEntry(_userId, _entry, _env);
                  await fetch("https://api.line.me/v2/bot/message/push", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${_token}` },
                    body: JSON.stringify({ to: _userId, messages: [{ type: "text", text: result.reply, quickReply: aicaBuildEmergencyQuickReply() }] }),
                  });
                  if (_env.SLACK_BOT_TOKEN) {
                    const nowJST = new Date().toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" });
                    await fetch("https://slack.com/api/chat.postMessage", {
...
- 600-800文字で作成`;
          ...
          } else if (nextPhase === "aica_turn1") {
            // Patch 7: AICA再開時のwelcome表示
            replyMessages = [{
              type: "text",
              text: aicaBuildWelcomeMessage(entry.aicaDisplayName),
              quickReply: aicaBuildWelcomeQuickReply(),
...
- 統計引用ルール（厳守）: 出典が確認できない統計データは絶対に引用しない。「厚労省○○調査」「○○年データ」等の権威付けを伴う数値は、ハルシネーションが起きやすいため使用禁止。不確かな場合は「公的調査では○○程度のレンジが報告されています」と幅で示す（出典の特定なし）。具体数値（賞与○ヶ月、月給○万円等）は施設DBの実データのみ使用。想像で数字を作ることは絶対禁止

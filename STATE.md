# ナースロビー 状態ファイル
# 最終更新: 2026-04-28 10:00 by 競合分析

## 🚨 次回起動時に必ず対応 — !reply 未送信3件（指示キューに滞留）

`data/slack_instructions.json` に「!reply プレフィックス付け忘れ」のLINE返信草稿が3件残っている。
**LINEには送信されていない**。送信するか破棄するか社長判断待ち。

| # | 日時 | userId（先頭8文字） | 本文の冒頭 |
|---|------|-------------------|-----------|
| 13 | 2026-04-26 14:27 | U7edff5f6 | 「石塚です。」 |
| 14 | 2026-04-26 17:28 | Ua3afee1a | 「ご登録ありがとうございます！」 |
| 15 | 2026-04-27 15:16 | U3dac5c67 | 「千恵子様、ナースロビーのキャリアアドバイザーの石塚と申します。」 |

- 原因: 先頭に `!reply` が無く、slack_commanderが指示キューに保存
- そのとき slack_commander.py はネットワークタイムアウトでリマインダー誤爆検知も動作不安定
- 社長から「まだ送らないで」指示済み（2026-04-27夜）→ 勝手にreplyするな

### 対応方法
- **A**: 社長/石塚さんが Slack で `!reply <userId> <本文>` を1件ずつ手動再送
- **B**: 私が指示後 `!reply` 代理実行（worker /push経由）
- **C**: #13/#14 は古いので破棄、#15（千恵子様）のみ送信

# ナースロビー 状態ファイル
# 最終更新: 2026-04-28 by LINE Bot v2.0 P0+P1+P2 実装デプロイセッション

## 🚀 2026-04-28 LINE Bot v2.0 大改修 (P0+P1+P2 11タスク完了)

### 背景
- Ayako Tarui事案（4/27）+ すずき りえこ問い合わせ（4/27-28）でBOT/人間競合問題が露呈
- 6人パネル討論で全体再設計: AI-First方針承認、M1-M5自動化、M6+のみ人間
- 設計書: `docs/audit/2026-04-28-line-system/MASTER-DESIGN-v2.md`
- 全体レポート: `docs/audit/2026-04-28-line-system/LINE-SYSTEM-FULL-REPORT.md`

### 実装完了 (P0+P1+P2)

**P0: 離脱防止・沈黙ガード** (Worker df9f518d)
- entry.humanRepliedAt 沈黙ガード（!reply送信後72hBOT沈黙）
- 「読み取れません」全廃→AI相談ルーティング
- nurture/apply/interview/faq/matching系→AI相談ルーティング
- 音声入力Whisper API（LINE音声→文字起こし→handleFreeTextInput）

**P1: AI相談強化・トリガー多様化** (Worker 8839ba2f)
- AI相談ターン 5→12 / 延長 8→18
- 会話コンテキストwindow 6→14
- entry.matchingResults+interestedFacility を実データ注入
- 共感ファースト・ハルシネ抑止プロンプト刷新
- 「気になる」直後のAI深掘り（5択QR、バックグラウンド生成）
- マッチング0件→AI条件再設計（relax=ai_help）

**P2: 自己分析エージェント+キャリアシート** (Worker 77001d9e)
- 7問インタビュー（経験/強み/動機/MUST/NG/未来/補足、音声OK、スキップ可）
- AIキャリアシート自動生成（gpt-4o-mini→Claude→静的の3段FB）
- ハルシネ対策3段（厳守ルール+禁止語11語検出+伏せ字）
- 応募意思表明ボタン M6（apply_intent=start→handoff→Slack通知）

### 連動した導線
- AI相談3往復後/ターン上限時 → 「📝履歴書を作る」「📨応募する」QR誘導
- sa=confirm → キャリアシートKV保存（member:{userId}:career_sheet）→ Slack📋通知
- M6到達 → 🎯 Slack通知（ここで初めて人間に通知）

### 保留・残課題
- ⏸ P1-4: 60秒沈黙proactive Push（cron頻度の制約でP3統合検討）
- ⏸ P3-1: モバイル管理ダッシュボード（セキュリティ設計が要るため別セッション）
- ⏸ #7: chat.line.biz禁止 vs LINE Operator API（保留中）
- ⏸ #8: AI相談 gpt-4o 昇格（月+¥30,000、保留中）

### 進捗
- TaskList: 11/14 完了（P0:4/4 + P1:3/4 + P2:4/4）
- Worker Versions: df9f518d → 8839ba2f → 77001d9e
- Commits: a2213c6 → 8111f0a → af52d2a

### 次のセッション
- P3-1 管理ダッシュボード（最優先）
  - URL: `https://quads-nurse.com/admin/`
  - HMAC+IP allowlist+admin secret 3層認証
  - 機能: 会話一覧/ユーザー詳細/BOT ON-OFF/返信/キャリアシート閲覧
  - PWA対応でスマホHome追加可
  - 工数: 5日見込
...
## ✅ 2026-04-26〜28 千恵子様（U3dac5c67f5cbaa77a580cc377b77f93e）対応完了

- 4/26 14:03 LINE友だち追加（直接フォロー、source/area/温度なし）
- 4/26 15:16 石塚さん：返信草稿をSlack投稿 → **`!reply` 抜け** で指示キューに滞留 → LINE未送信
- 4/26 16:06 再投稿：``` ```!reply userid``` ``` でトリプルバッククォートで囲ってしまい再度失敗（regex不一致）
- 4/26 16:08 **`!reply U3dac5c67... <本文>` 形式で正しく投稿 → LINE送信完了**（bot応答 `✅ LINE送信完了`、ts=1777184912）
- 千恵子様からの返信待ち（24時間ルール監視、Slack #ロビー小田原人材紹介で受信通知）

### 千恵子様への提示用：堺市求人 詳細リスト作成済み
- ファイル: `data/job_briefs/sakai_2026-04-27.md`（648行、37KB）
- 堺市堺区 全23件 + 堺市北区 全20件、ハローワークAPI/D1取得
- 各求人に: 施設・職種・勤務地・給与・賞与・年休・最寄駅・勤務時間・契約期間・社会保険・福利厚生・仕事内容
- セールスポイント早見表付（年収/年休/賞与/夜勤NG/訪看/病院/特養/ブランクOK/パート高単価別）
- 北区Aランクは **正風病院（医療法人紀和会、年休127日）** のみ

### slack_commander.py の `!reply` パース仕様（再発防止メモ）
- **必須**: 1行目が `!reply <userID> <本文先頭>` の形（半角スペース区切り）
- **NG例1**: 先頭にuserIDだけ → 指示キューに保存され LINE 未送信
- **NG例2**: ``` ```!reply userid``` ``` でバッククォート囲み → regex `[Uu][0-9a-f]{32}[\s　]+([\s\S]+)` が直後の `` ` `` で不一致 → 「使い方:」エラー
- **正しい形**: `!reply U3dac5c6... 千恵子様\n\nナースロビーの…`（userIDの後に半角スペース、本文は改行可）
- 詳細: scripts/slack_commander.py:663-688

### 過去の未送信指示（#13/#14）の扱い
- 4/20 14:27 #13 U7edff5f6 「石塚です。」→ status: done（手動done化済、LINE送信は別途確認要）
- 4/20 17:28 #14 Ua3afee1a 「ご登録ありがとうございます！」→ status: done（同上）
- いずれも `data/slack_instructions.json` ベース。実LINE送信履歴は Slackで `!reply` コマンド検索で確認可

---

## 🚨 2026-04-27 15:00 コンテンツ生成バッチ全滅（Anthropic Opusダウン）

### 状況
- cron `pdca_content.sh` 15:00実行中、Anthropic側でOpus障害
- claude CLI が exit 1 連発 → 7件全て失敗（0/7生成）
- Slackに [OPUS DOWN] アラート 36回連発（リトライ毎）
- CFフォールバックは品質維持のため無効化済み（仕様通り）

### 影響
- **当日投稿は無事**: 既存キュー 38件 ready で17:00以降のSNS投稿は通常通り
- 緊急度: 低（明日15:00にcronが再走行）

### 復旧確認
- 2026-04-27 15:31 claude CLI で `1+1=` テスト → "2" 正常応答
- 再生成するかは社長判断（Slack報告済み、未指示）

---

## 🌏 2026-04-26 全国47都道府県対応 完了

### 背景
- 大阪から登録が発生 → 「神奈川集中」では受け皿不足
- 6人パネル討論で戦略策定 → ナースロビー全国区ブランドとして整理

### 完了事項
- **施設DB全国拡張**: 厚労省egov CSV → D1に84,613施設投入（全47都道府県、東京14,711/大阪7,567/愛知4,719/福岡4,735ほか）
- **求人取得スクリプト**: ハローワーク `hellowork_fetch.py --all-japan` で47都道府県対応（M101-M147）
- **求人データ取得**: バックグラウンド実行中（17/47県完了）。完了後 `hellowork_to_d1.py` で D1 jobs テーブル投入予定
- **Worker**: AREA_PREF_MAP/PREF_NAMEを47県展開、JAPAN_PREFECTURES + JAPAN_REGIONS定数追加
- **LINE Bot UI**: 「他の都道府県（全国対応）」→地方ブロック→都道府県の2段階ピッカー
- **順番待ち実装**: `waitlist:{userId}` KV保存、都道府県聞き取り、Slack通知、unfollow自動cleanup
- **管理API**: `/api/admin/trigger-waitlist-push` で都道府県別バッチPush可能
- **Worker デプロイ済み** (Version: 6e1a55df-c1e8-44ff-aad3-8b954a7e3824)

### 社長確認待ち
1. **許可証23-ユ-302928の「取扱地域」記載**: 「神奈川県」等に限定されている場合は神奈川労働局へ「取扱地域変更届」提出が必要（添付書類不要、即日反映）。「全国」または未記載なら追加手続き不要
2. **LP内の神奈川訴求文言**: FAQ「神奈川県以外も対応していますか？」回答とセクションタイトル「神奈川の看護師求人」「神奈川の看護師転職情報」を全国対応化するか維持か（SEOロングテール影響あり）

### 悪魔の警告（無視するな）
- 求人DBゼロのエリアに広告を1円でも使うな（機会破壊）→ 大阪等の広告配信は契約病院3件確保まで凍結
- 「全国対応」を匂わせる文言を必要以上に書くな（期待値=離脱率）
- SEO 305ページ（area21+guide41+blog18+hospitals）の「神奈川ナース転職」維持が地名ロングテール武器

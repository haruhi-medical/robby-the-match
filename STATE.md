# ナースロビー 状態ファイル
# 最終更新: 2026-04-03 11:30 by リブランド+LIFF実装
...
- 状態: **v3設計書に基づくリブランド+LINE刷新 進行中**

## 全体設計書との差分（次回ここから再開）

### ✅ 完了済み（9+5+2件）
- 共通LINE送客EP `/api/line-start` + LP全CTA統一
- Worker welcome分岐6パターン
- intake_light 3問（エリア/働き方/温度感）→ matching
- Hero直下 安心バー
- GA4イベント統一（LP側 click_cta）/ Meta Lead二重計測修正
- 誤入力3段階 / AI相談ターン上限5 / OpenAI失敗日本語FB
- matching_browse（他の求人も見たい）
- ナーチャリングCron Day3/7/14/30
- ハンドオフ後Bot補助（2h follow-up）
- LP shindan.js 7問→3問化
- **LP構成変更**: 安心バー→診断→Features→自分向け感→比較→Flow→FAQ→CTA
- **求人スコアリング5軸**: エリア/働き方/給与/施設タイプ/条件マッチ + 推薦理由バッジ表示
- **計測設計12イベント**: KV日次カウント + Meta CAPI/GA4 MP対応（トークン設定で有効化）
- **リッチメニュー4状態**: default/hearing/matched/handoff（メニューID設定で有効化）
- **ハンドオフ5条件判定**: 温度感A/求人詳細/逆指名/本人相談/5ターン + 直近会話表示
- **state正式定義16種**: 12カテゴリ×全phase + 条件緩和提案（マッチ3件未満時）

### 🟢 設計書Step 1〜6 全完了！

### ✅ 2026-04-03 実施（リブランド+LIFF）
- **リブランド「ナースロビー」**: LP/Worker/config/広告LP/about/salary-check 全面適用（SEOページは維持）
- **CTA改善**: 「LINEで無料相談」→「あなたに合う求人を見る」「診断結果を受け取る」、LINEアイコン除去
- **不安除去コピー**: 「いつでもブロックOK」「通知は最小限」
- **LIFF実装**: LIFFブリッジページ + /api/link-session + follow時セッション復元 + 既友だちPush対応
- **LIFF ID**: 2009683996-7pCYfOP7（テスト成功済み）

### ✅ 2026-04-03 追加実施
- **worker.js状態削減**: 50→20状態（858行削除）
- **アキネーター型UI**: 候補数リアルタイム表示 + 施設タイプ質問追加（3問→4問）
- **AI多層フォールバック**: OpenAI→Claude Haiku→Gemini Flash→Workers AI（4段階）
- **SEO全85ファイル**: CTAをLIFFブリッジ経由に変更 + LINE文言除去

### 🔴 次にやること
| Step | 内容 | 状態 |
|------|------|------|
| 1 | Meta広告クリエイティブをナースロビーに更新 | 未着手（社長手動） |
| 2 | 広告予算増額テスト + 実測CPA取得 | 未着手 |
| 3 | e-Gov CSV → D1インポート（18万施設DB） | 未着手（中期） |
| 4 | AnthropicキーとGoogleキーをWorkerシークレットに追加 | 未着手（AI多層有効化） |

### ⚠️ パネル討論で出た改善案（未適用・保留中）
- intake_light 3問→2問（温度感削除）
- matching_preview廃止→即Flexカルーセル
- Quick Reply 4択→2択
→ 設計書に沿った実装が完了してから再検討する

# ナースロビー 状態ファイル
# 最終更新: 2026-04-03 22:00 by v3設計書大規模実装セッション
...
| # | 内容 | 工数 |
|---|------|------|
| 1 | **LINE Botに診療科フィルタ追加**（「希望診療科は？」質問） | 1日 |
| 2 | **カード5枚+末尾ナビカード**（「他を探す/相談する」） | 半日 |
| 3 | **施設コメント自動生成**（客観ファクト: 駅○分/○床/託児所等） | 半日 |

### 優先度B（DB品質+運用）
| # | 内容 | 工数 |
|---|------|------|
| 4 | 看護師数カバー率100%に近づける（残200-300施設） | 1日 |
| 5 | 介護CSV構造確認（D1投入は成約後） | 2h |
| 6 | Anthropic/GoogleキーをWorker secretsに追加 | 社長手動 |

### 優先度C（広告+集客）
| # | 内容 | 工数 |
|---|------|------|
| 7 | Meta広告クリエイティブをナースロビーに更新 | 社長手動 |
| 8 | 広告予算増額テスト + 実測CPA取得 | 継続 |

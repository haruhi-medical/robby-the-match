#!/usr/bin/env python3
"""
背景画像に日本語テキストを合成 v2.0
SNSインフルエンサー3人の改善案を反映
"""
import json
from pathlib import Path
from PIL import Image, ImageDraw, ImageFont
import re

# ディレクトリ設定
project_dir = Path("/Users/robby2/robby_content/post_001")
backgrounds_dir = project_dir / "backgrounds"
output_dir = project_dir / "final_slides_v2"
output_dir.mkdir(exist_ok=True)

# slide_prompts.jsonを読み込み
with open(project_dir / "slide_prompts.json", "r", encoding="utf-8") as f:
    data = json.load(f)

# 日本語フォントを探す
font_paths = [
    "/System/Library/Fonts/Hiragino Sans GB.ttc",
    "/Library/Fonts/Arial Unicode.ttf",
]

japanese_font_path = None
for path in font_paths:
    if Path(path).exists():
        japanese_font_path = path
        print(f"✅ 日本語フォント検出: {path}")
        break

if not japanese_font_path:
    print("❌ エラー: 日本語フォントが見つかりません")
    exit(1)

def parse_text_with_highlights(text):
    """
    テキストから強調部分を抽出
    slide_prompts.jsonのtext_styleに基づいて黄色で強調する部分を識別
    """
    # 強調キーワードのパターン
    highlight_keywords = [
        "担当が焦った", "これが限界です", "AIに聞いてみた",
        "▲60-100万円低い", "480-520万円", "1.5万円/回",
        "再確認してみます", "やってくれよ", "プロフのリンクから"
    ]

    # 数字パターン
    number_patterns = [
        r'\d+[-~]\d+万円',  # 480-520万円
        r'▲\d+[-~]\d+万円', # ▲60-100万円
        r'\d+\.\d+万円',    # 1.5万円
        r'\d+万',           # 420万
    ]

    return highlight_keywords, number_patterns

def draw_text_with_highlight(draw, text, x, y, font, default_color=(255, 255, 255),
                              highlight_color=(255, 215, 0), highlight_keywords=None, number_patterns=None):
    """
    テキストを描画し、キーワードを強調表示
    """
    if highlight_keywords is None:
        highlight_keywords = []
    if number_patterns is None:
        number_patterns = []

    # キーワードが含まれているかチェック
    is_highlighted = False
    for keyword in highlight_keywords:
        if keyword in text:
            is_highlighted = True
            break

    # 数字パターンをチェック
    if not is_highlighted:
        for pattern in number_patterns:
            if re.search(pattern, text):
                is_highlighted = True
                break

    # チェックマーク（✓）が含まれる場合も強調
    if '✓' in text:
        is_highlighted = True

    color = highlight_color if is_highlighted else default_color
    draw.text((x, y), text, font=font, fill=color)

    return is_highlighted

print("=" * 60)
print("テキストオーバーレイ合成スタート v2.0")
print("=" * 60)
print()

# まずスライド1だけテスト生成
slide = data["slides"][0]
slide_num = slide["slide_number"]
text_overlay = slide["text_overlay"]

print(f"[テスト] スライド{slide_num}を処理中...")

# 背景画像を読み込み
bg_file = backgrounds_dir / f"slide_{slide_num:02d}_bg.png"
if not bg_file.exists():
    print(f"❌ 背景画像が見つかりません: {bg_file}")
    exit(1)

img = Image.open(bg_file)
width, height = img.size

# フォントサイズを計算（画面幅の1/12 ≈ 85px）
base_font_size = width // 12
title_font_size = int(base_font_size * 1.5)

try:
    base_font = ImageFont.truetype(japanese_font_path, base_font_size)
    title_font = ImageFont.truetype(japanese_font_path, title_font_size)
except Exception as e:
    print(f"⚠️ フォント読み込み失敗: {e}")
    base_font = ImageFont.load_default()
    title_font = ImageFont.load_default()

# テキストを行ごとに分割
lines = text_overlay.split("\n")

# 強調キーワードとパターンを取得
highlight_keywords, number_patterns = parse_text_with_highlights(text_overlay)

# テキストの総高さを計算
total_height = 0
for i, line in enumerate(lines):
    if i == 0:  # 最初の行はタイトル
        total_height += title_font_size * 1.3
    else:
        total_height += base_font_size * 1.5

# 半透明の黒帯を描画（中央に配置）
overlay_y_start = (height - int(total_height)) // 2
overlay_y_end = overlay_y_start + int(total_height) + 60  # 上下余白30pxずつ

# 半透明黒帯用の一時レイヤー（透明度150）
overlay = Image.new('RGBA', img.size, (0, 0, 0, 0))
overlay_draw = ImageDraw.Draw(overlay)
overlay_draw.rectangle(
    [(0, overlay_y_start), (width, overlay_y_end)],
    fill=(0, 0, 0, 150)  # 半透明黒 (alpha=150、背景がほんのり見える)
)

# 画像に合成
img = img.convert('RGBA')
img = Image.alpha_composite(img, overlay)
img = img.convert('RGB')
draw = ImageDraw.Draw(img)

# テキストを描画
y_offset = overlay_y_start + 30  # 上余白30px
for i, line in enumerate(lines):
    # 空行はスキップ
    if not line.strip():
        y_offset += base_font_size // 3
        continue

    # フォント選択（最初の行はタイトル）
    font = title_font if i == 0 else base_font

    # テキストの幅を取得
    try:
        bbox = draw.textbbox((0, 0), line, font=font)
        text_width = bbox[2] - bbox[0]
    except:
        text_width = len(line) * (title_font_size if i == 0 else base_font_size) // 2

    # 中央寄せ
    x = (width - text_width) // 2

    # テキストを描画（キーワード強調あり）
    draw_text_with_highlight(
        draw, line, x, y_offset, font,
        default_color=(255, 255, 255),
        highlight_color=(255, 215, 0),  # ゴールド
        highlight_keywords=highlight_keywords,
        number_patterns=number_patterns
    )

    y_offset += int((title_font_size if i == 0 else base_font_size) * 1.5)

# 保存
output_file = output_dir / f"slide_{slide_num:02d}_final_v2.png"
img.save(output_file, "PNG")

print(f"✅ 保存完了: {output_file.name}")
print()
print("=" * 60)
print("✅ テスト画像生成完了！")
print("=" * 60)
print(f"保存先: {output_file}")
print()
print("確認して問題なければ、全6枚を生成できます。")

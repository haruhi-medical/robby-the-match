...
def get_next_from_queue(platform: str) -> Optional[Path]:
    """Get next ready item from posting_queue.json with valid slides.

    This is the primary content source (batch-generated carousels with text).
    Falls back to content/ready/ directories if queue is empty.
    """
    if not QUEUE_FILE.exists():
        return get_next_unposted_legacy(platform)
    ...
    for post in q.get("posts", []):
        ...
        if slide_dir.name in posted_dirs:
            continue
        print(f"[QUEUE] Selected from posting_queue: {post.get('content_id')} ({len(pngs)} slides)")
        return slide_dir

    # Fallback to legacy content/ready/ system
    print("[QUEUE] No ready items in posting_queue, falling back to content/ready/")
    return get_next_unposted_legacy(platform)


def get_next_unposted_legacy(platform: str) -> Optional[Path]:
    """Legacy: Get next unposted content directory from content/ready/.
...
def get_next_unposted(platform: str) -> Optional[Path]:
    """Get next content to post. Tries posting_queue first, then content/ready/."""
    return get_next_from_queue(platform)
...
def prepare_instagram_slides(content_dir: Path) -> Optional[List[Path]]:
    ...
    # Fallback: check if slide_*.png/jpg exist in content_dir itself
    # (may have been generated by a previous run outside the instagram/ subdir)
    fallback_slides = sorted(content_dir.glob("slide_*.png")) or sorted(content_dir.glob("slide_*.jpg"))
    if fallback_slides:
        print(f"[IG-PREP] Found {len(fallback_slides)} existing slides in content_dir (fallback)")
...
def post_to_instagram(content_dir: Path, dry_run: bool = False,
    ...
    if ig_slides:
        ...
        # Fall back to existing slides in content_dir
        # Match both "slide_*.png" and "*_slide_*.png" patterns
        slides = sorted(content_dir.glob("*slide*.png"))
        if not slides:
            slides = sorted(content_dir.glob("*slide*.jpg"))
        if slides:
            print(f"[IG] Using existing slides from {content_dir.name} ({len(slides)} slides)")
        else:
            # No proper slides found — try to generate from JSON content file
            print("[IG] No slide_*.png/jpg found. Attempting to generate from JSON content...")
            json_files = [f for f in content_dir.glob("*.json") if f.name != "schedule.json"]
            if json_files:
                carousel_script = PROJECT_DIR / "scripts" / "generate_carousel.py"
                if carousel_script.exists():
                    try:
                        result = subprocess.run(
                            [sys.executable, str(carousel_script),
                             "--single-json", str(json_files[0]),
                        ...
                        if result.returncode == 0:
                            slides = sorted(content_dir.glob("slide_*.png")) or sorted(content_dir.glob("slide_*.jpg"))
                            if not slides:
                                # Check subdirectories
                                slides = sorted(content_dir.glob("**/slide_*.png")) or sorted(content_dir.glob("**/slide_*.jpg"))
                        if result.returncode != 0:
                            print(f"[IG] generate_carousel.py failed (exit={result.returncode})")
                            if result.stderr:
                                print(f"[IG] stderr: {result.stderr[:300]}")
                    except Exception as e:
                        print(f"[IG] generate_carousel.py error: {e}")
            if slides:
                print(f"[IG] Generated {len(slides)} slides from JSON content")
            else:
                return {"status": "error", "error": "No slides found and generation failed"}

    # --- Validate slide file sizes (detect blank/background-only images) ---
    MIN_SLIDE_SIZE_KB = 30
    has_valid_slide = any(s.stat().st_size > MIN_SLIDE_SIZE_KB * 1024 for s in slides)
    if not has_valid_slide:
        total_sizes = ", ".join(f"{s.name}={s.stat().st_size // 1024}KB" for s in slides)
        print(f"[IG] WARNING: All slides are under {MIN_SLIDE_SIZE_KB}KB — likely blank/background-only images")
        print(f"[IG] Slide sizes: {total_sizes}")
        return {"status": "error", "error": f"All slides under {MIN_SLIDE_SIZE_KB}KB — likely textless backgrounds"}
...
def post_next(platforms: List[str], dry_run: bool = False,
    ...
    # Update posting_queue.json status for successful posts
        ...
        try:
            ...
            for r in results:
                ...
                for post in q.get("posts", []):
                    ...
                    if dir_name in sd or sd.endswith(dir_name):
                        post["status"] = "posted"
                        post["posted_at"] = datetime.now().isoformat()
                        post["instagram_dir"] = dir_name
                        print(f"[QUEUE] Marked {post.get('content_id')} as posted")
                        break
            with open(QUEUE_FILE, "w") as f:
                json.dump(q, f, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"[QUEUE] Failed to update posting_queue: {e}")

...
## スライド構成（8枚 — 映像作品として設計）
1枚目: Hook（フック） — 看護師が「自分のことだ」と即わかる10文字以内。
  フックに「看護師」「夜勤」「病棟」「師長」「手取り」等の現場語を必ず含めろ。
  「ロビー」「AI」は入れるな。知らない人にとって意味不明。
...
## フックの型（ローテーション必須。全て看護師の現場語で）
1. 矛盾型: 「夜勤明け、辞められない」
2. 断片型: 「手取り24万。命の値段」
3. 告白型: 「3ヶ月、師長に黙ってた」
4. 対比型: 「看護記録72分 vs 10分」
5. 沈黙型: 「師長が何も言わなかった」
6. 問い型: 「看護師3年目、限界？」
7. 衝撃数字型: 「紹介料80万の行方」

## ルール
- フックは10文字以内。看護師が「自分のことだ」と即座にわかること
...
- 口調は「〜だよ」「〜なんだ」。敬語禁止。
- フック（1枚目）にはロビーの名前を入れるな。看護師の現場語だけで書け。
- 2枚目以降の解説文で「ロビー」を自然に使え（3-4文に1回程度）。
...
- hook: 1枚目（10文字以内。看護師が「自分のことだ」と即わかる現場語。「ロビー」は入れるな）
- slides: 8枚分のテキスト（配列）
    ...
    for attempt in range(2):
        ...
        if _validate_content(data, content_id):
            ...
            if ROBBY_LOADED:
                ...
                # フックに「ロビー」が含まれていないか確認（入れるべきでない）
                if "ロビー" in data.get("hook", ""):
                    print(f"  [VOICE] WARN: フックに「ロビー」が入っている。看護師の現場語に書き直すべき。")

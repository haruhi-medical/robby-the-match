...
class LineClient:
    ...
    # ------------------------------------------------------------------
    # 監査用エンドポイント (audit-snapshot / audit-reset)
    # ------------------------------------------------------------------

    def audit_snapshot(self, user_id: str, secret: str) -> Dict[str, Any]:
        """``/api/admin/audit-snapshot`` で entry KV state を取得。

        Args:
            user_id: 対象 userId（``U_TEST_`` prefix 推奨）。
            secret: ``AUDIT_SECRET`` または ``LINE_PUSH_SECRET``。

        Returns:
            ``{"status": int, "body": dict|str, ...}``
        """
        body = json.dumps({"userId": user_id, "secret": secret}).encode("utf-8")
        headers = {
            "Content-Type": "application/json",
            "User-Agent": "Mozilla/5.0 (LineClient)",
        }
        return self._post("/api/admin/audit-snapshot", body, headers)

    def audit_reset(self, user_id: str, secret: str) -> Dict[str, Any]:
        """``/api/admin/audit-reset`` で entry/関連KV を全削除。

        Worker側で ``U_TEST_`` / ``Utest`` prefix のみ受付（本番userId保護）。
        """
        body = json.dumps({"userId": user_id, "secret": secret}).encode("utf-8")
        headers = {
            "Content-Type": "application/json",
        ...
        return self._post("/api/admin/audit-reset", body, headers)

    # ------------------------------------------------------------------
    # async ラッパ (asyncio.to_thread で同期版を非同期化)
    # ------------------------------------------------------------------
    ...
    async def send_text_async(
        ...
        """``send_text`` の非同期版。"""
        return await asyncio.to_thread(self.send_text, user_id, text, reply_token)

    async def send_postback_async(
        self,
        user_id: str,
        data: str,
        reply_token: Optional[str] = None,
        ...
        return await asyncio.to_thread(
            self.send_postback, user_id, data, reply_token, params
        )
    ...
    async def send_audio_async(
        ...
        """``send_audio`` の非同期版。"""
        return await asyncio.to_thread(
            self.send_audio, user_id, audio_path, reply_token, duration_ms
        )

    async def send_follow_async(
        self,
        user_id: str,
        reply_token: Optional[str] = None,
    ) -> Dict[str, Any]:
        """``send_follow`` の非同期版。"""
        return await asyncio.to_thread(self.send_follow, user_id, reply_token)

    async def send_unfollow_async(self, user_id: str) -> Dict[str, Any]:
        """``send_unfollow`` の非同期版。"""
        return await asyncio.to_thread(self.send_unfollow, user_id)

    async def audit_snapshot_async(self, user_id: str, secret: str) -> Dict[str, Any]:
        """``audit_snapshot`` の非同期版。"""
        return await asyncio.to_thread(self.audit_snapshot, user_id, secret)

    async def audit_reset_async(self, user_id: str, secret: str) -> Dict[str, Any]:
        """``audit_reset`` の非同期版。"""
        return await asyncio.to_thread(self.audit_reset, user_id, secret)

...
【出力形式】
志望動機本文（600文字以内、一段落〜三段落）の後に、
必ず以下の引用セクションを付けて返してください（Markdown・JSONは禁止）。

本文

---
※ この文章は、お話しいただいた以下の言葉を基に作成しました。
「{候補者の実発言1}」
「{候補者の実発言2}」
（必要なら3つ目まで）
...
・候補者が本音ヒアリングで語った「根本的な悩み」や「譲れない条件」と、
  志望先がそれをどう満たせそうかを紐づける
・嘘は書かない。情報がない強みは書かない
・誇張表現（「必ず貢献できる自信があります」等）は避け、具体例で語る
・「御院」「貴院」どちらでも可。統一すれば良い
・絵文字は使わない
・引用は必ず【候補者の実発言】セクションから、本文で根拠として使ったものを
  2-3件、そのままの言葉で引用する（改変禁止。省略は「…」で可）
...
export async function runDocumentGeneration({ candidateId, env, db }) {
  ...
  // 会話履歴から候補者の実発言を抽出（志望動機フッター用）
  const candidateQuotes = await extractCandidateQuotes(env.AICA_DB, candidateId);
  const dataBlob = buildDataBlob(candidate, profile, employer, candidateQuotes);
  ...
  try {
    ...
    const [resume, career, motivation] = await Promise.all([
      ...
      generateOne({ systemPrompt: MOTIVATION_PROMPT.replace("{employer}", employer), dataBlob, env, maxTokens: 900 }),
    ]);
...
/**
 * 候補者の実発言を志望動機フッター用に抽出
 * - role='user' のメッセージ
 * - 10文字以上（Quick Reply選択テキストは短いので除外しやすい）
 * - 心理ヒアリング系（turn1-4, summary, condition_hearing）を優先
 * - 音声マーカー [voice] プレフィックスは削る
 */
async function extractCandidateQuotes(db, candidateId) {
  try {
    ...
    for (const r of rows) {
      ...
      quotes.push({ text: t, phase: r.phase });
    }
    const important = quotes.filter((q) =>
      ["turn1", "turn2", "turn3", "turn4", "summary", "condition_hearing"].includes(q.phase)
    );
    const pool = important.length >= 3 ? important : quotes;
    return pool.slice(0, 10).map((q) => q.text);
  } catch (err) {
    console.warn("[doc-gen] extractCandidateQuotes failed:", err.message);
    return [];
  }
}

/** AI に渡すデータブロブを組み立て */
function buildDataBlob(candidate, profile, employer, candidateQuotes = []) {
  const lines = [];
  lines.push("【候補者情報】");
  ...
  if (candidateQuotes.length > 0) {
    lines.push("");
    lines.push("【候補者の実発言（引用可能な発言群）】");
    candidateQuotes.forEach((q, i) => {
      lines.push(`  ${i + 1}) ${q}`);
    });
  }
  return lines.join("\n");
